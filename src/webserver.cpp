...
void _onPostConfigFile(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    ...
    // No buffer
    if (final && (index == 0)) {
        //_webConfigSuccess = settingsRestoreJson((char*) data); //WEB_SUPPORT
        return;
    }
    ...
    // Copy
    if (len > 0) {
        if ((_webConfigBuffer->size() + len) > std::min((uint32_t)WEB_CONFIG_BUFFER_MAX, (uint32_t)(ESP.getFreeHeap() - sizeof(std::vector<uint8_t>)))) {
    ...
    if (final) {
        ...
        //_webConfigSuccess = settingsRestoreJson((char*) _webConfigBuffer->data()); //WEB_SUPPORT
        delete _webConfigBuffer;
...
void webLog(AsyncWebServerRequest *request) {
    DEBUG_MSG_P(PSTR("[WEB] Request: %s %s\n"), request->methodToString(), request->url().c_str());
}
...
void webSetup() {
    ...
    // Run server
        ...
        {
            if ((code == MESSAGE_CONNECTED) || (MESSAGE_ACCESSPOINT_CREATED == code) )
            {
                _server->begin();
                DEBUG_MSG_P(PSTR("[WEB] Webserver listening at port %u\n"),webPort());
            }
        });
    #endif
}

...
void _wifiUpdateSoftAP() {
    if (WiFi.softAPgetStationNum() == 0) {
            ...
            jw.setSoftAP(getSetting("hostname").c_str(), getAdminPass().c_str(),"172.16.16.1", "172.16.16.1", "255.255.255.0");
        #else
...
void _wifiConfigure() {
    ...
    WiFi.setSleep(static_cast<wifi_ps_type_t>(sleep_mode));

    #if WIFI_GRATUITOUS_ARP_SUPPORT
        _wifi_gratuitous_arp_last = millis();
        _wifi_gratuitous_arp_interval = getSetting("wifiGarpIntvl", random( //secureRandom(
            WIFI_GRATUITOUS_ARP_INTERVAL_MIN, WIFI_GRATUITOUS_ARP_INTERVAL_MAX
        )).toInt();
    #endif

    const auto tx_power = getSetting("wifiTxPwr", WIFI_POWER_19_5dBm).toInt();
    WiFi.setTxPower(static_cast<wifi_power_t>(tx_power));
}
...
void _wifiScan(wifi_scan_f callback = nullptr) {
    ...
    for (unsigned char i = 0; i < result; ++i) {
        ...
        WiFi.getNetworkInfo(i, info.ssid_scan, info.sec_scan, info.rssi_scan, info.BSSID_scan, info.chan_scan);
        ...
        snprintf_P(info.buffer, sizeof(info.buffer),
            ...
            (info.sec_scan != WIFI_AUTH_OPEN ? "YES" : "NO "),
            info.rssi_scan,
            info.chan_scan,
...
void _wifiWebSocketOnAction(uint32_t client_id, const char * action, JsonObject& data) {
    if (strcmp(action, "scan") == 0) wsPost(client_id, _wifiWebSocketScan); //WEB_SUPPORT
}
...
String _wifiSoftAPSSID() 
 {
    ...
    return String(reinterpret_cast<const char*>(config.ap.ssid)); //???
}
...
String _wifiSoftAPPSK() 
{
    wifi_config_t config;
    esp_wifi_get_config(WIFI_IF_AP, &config);
    return String(reinterpret_cast<const char*>(config.ap.password));
}
...
void wifiDebug(WiFiMode_t modes) {
    ...
    if (((modes & WIFI_STA) > 0) && ((WiFi.getMode() & WIFI_STA) > 0)) {
        ...
        DEBUG_MSG_P(PSTR("[WIFI] MODE  Station\n"));
    ...
    if (((modes & WIFI_AP) > 0) && ((WiFi.getMode() & WIFI_AP) > 0)) {
        DEBUG_MSG_P(PSTR("[WIFI] MODE  Access Point\n"));
...
void wifiSetup() {
    ...
    // Main callbacks
    espurna32RegisterLoop(wifiLoop);
    espurna32RegisterReload(_wifiConfigure);

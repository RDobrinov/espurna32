...
void _haSendDiscovery() {
    ...
    //const unsigned long ts = millis(); //SKIPPED
    /*do {
...
void _haWebSocketOnAction(uint32_t client_id, const char * action, JsonObject& data) {
    if (strcmp(action, "haconfig") == 0) {
        ...
        if (callbacks.size()) wsPostSequence(client_id, callbacks);
    }
}
...
void haSetup() {
    ...
    // Main callbacks
    espurna32RegisterReload(_haConfigure);

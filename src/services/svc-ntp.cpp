...
int inline _ntpSyncInterval() {
    return random(NTP_SYNC_INTERVAL, NTP_SYNC_INTERVAL * 2);
}

int inline _ntpUpdateInterval() {
    return random(NTP_UPDATE_INTERVAL, NTP_UPDATE_INTERVAL * 2);
}
...
void ntpSetup() {
    ...
    wifiRegister([](justwifi_messages_t code, char * parameter) {
        if (code == MESSAGE_CONNECTED) {
            if (!ntpSynced()) {
                _ntp_defer.once_ms(random(NTP_START_DELAY, NTP_START_DELAY * 15), _ntpWantSync);
            }
        }
    });
    ...
    // Main callbacks
    espurna32RegisterLoop(_ntpLoop);
    espurna32RegisterReload([]() { _ntp_configure = true; });

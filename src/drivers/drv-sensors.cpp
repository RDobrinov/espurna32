...
#include "drivers/sensors/SensorConstatns.hpp"
...
void _sensorLoad() {
    ...
    #if BMX280_SUPPORT
    {
        if (BMX280_I2CPORT_A != I2C_NUM_MAX)
        {
            ...
            const unsigned char address_map[2] = { BMX280_ADDRESS_1, BMX280_ADDRESS_2 };

            for (unsigned char n=0; n < number; ++n) {
                BMX280Sensor * sensor = new BMX280Sensor();
                sensor->setAddress(address_map[n], BMX280_I2CPORT_A);
                _sensors.push_back(sensor);
            }
            
        }
        if (BMX280_I2CPORT_B != I2C_NUM_MAX)
        {
            const unsigned char number = constrain(getSetting("bmx280Number", BMX280_NUMBER).toInt(), 1, 2);
            ...
            for (unsigned char n=0; n < number; ++n) {
                BMX280Sensor * sensor = new BMX280Sensor();
                sensor->setAddress(address_map[n], BMX280_I2CPORT_B);
                _sensors.push_back(sensor);
            }
    ...
    #if RTD_SUPPORT
        MAX31865Sensor * sensor = new MAX31865Sensor();
        _sensors.push_back(sensor);
    #endif
...
void _sensorInit() {
    ...
    for (unsigned char i=0; i<_sensors.size(); i++) {
        ...
        String strDescription = _sensors[i]->description();
        ...
        for (unsigned char k=0; k<_sensors[i]->count(); k++) {
            ...
            DEBUG_MSG_P(PSTR("[SENSOR] %s -> %s:%d\n"), strDescription.c_str(), magnitudeTopic(type).c_str(), _counts[type]);
...
void sensorSetup() {
    ...
    // Main callbacks
    espurna32RegisterLoop(sensorLoop);
    espurna32RegisterReload(_sensorConfigure);

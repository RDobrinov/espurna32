...
bool relayStatus(unsigned char id, bool status) {
    //return true;
    #if MQTT_SUPPORT
        return relayStatus(id, status, mqttForward(), true); //SKIPPED ERROR
    #else
...
void relayToggle(unsigned char id) {
    relayToggle(id, mqttForward(), true); //SKIPPED ERROR
}
...
void relaySetup() {
    ...
    // Main callbacks
    espurna32RegisterLoop(_relayLoop);
    espurna32RegisterReload(_relayConfigure);

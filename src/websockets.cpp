...
bool _wsStore(const String& key, const String& value) {
    //DEBUG_MSG_P(PSTR("[WSOCK] _wsStore %s: %s\n"), key.c_str(), value.c_str());
    if (key == "webPort") {
    ...
    if (value != getSetting(key)) {
        //DEBUG_MSG_P(PSTR("[WSOCK] _wsStore SET key %s with value %s\n"), key.c_str(), value.c_str());
...
void _wsParse(AsyncWebSocketClient *client, uint8_t * payload, size_t length) {
    ...
    if (!root.success()) {
        DEBUG_MSG_P(PSTR("[WSOCK] JSON parsing error\n"));
    ...
    if (action) {
        ...
        DEBUG_MSG_P(PSTR("[WSOCK] Requested action: %s\n"), action);
    ...
    if (config.success()) {
        ...
        DEBUG_MSG_P(PSTR("[WSOCK] Parsing configuration data\n"));
        ...
        for (auto kv: config) {
            ...
            //DEBUG_MSG_P(PSTR("[JSON] %s, %s\n"),key.c_str(), value.as<char*>());
...
void _wsOnConnected(JsonObject& root) {
    char chipid[7];
    snprintf_P(chipid, sizeof(chipid), PSTR("%06X"), getChipId());
    ...
    //root["btnDelay"] = getSetting("btnDelay", BUTTON_DBLCLICK_DELAY).toInt(); //WEB_SUPPORT
    ...
    if (type == WS_EVT_CONNECT) {
        ...
        client->_tempObject = nullptr;
            ...
            if (!_wsAuth(client)) {
                ...
                DEBUG_MSG_P(PSTR("[WSOCK] Validation check failed\n"));
                client->close();
                return;
            }
        #endif
        ...
        IPAddress ip = client->remoteIP();
        DEBUG_MSG_P(PSTR("[WSOCK] #%u connected, ip: %d.%d.%d.%d, url: %s\n"), client->id(), ip[0], ip[1], ip[2], ip[3], server->url());
        ...
        DEBUG_MSG_P(PSTR("[WSOCK] #%u disconnected\n"), client->id());
        ...
        DEBUG_MSG_P(PSTR("[WSOCK] #%u error(%u): %s\n"), client->id(), *((uint16_t*)arg), (char*)data);

    } else if(type == WS_EVT_PONG) {
        DEBUG_MSG_P(PSTR("[WSOCK] #%u pong(%u): %s\n"), client->id(), len, len ? (char*) data : "");
